...
4. The actor runtime `fetch` callback extracts those headers inside the Durable Object and creates the Durable
   Object-side server span. It also creates the socket session id and persists the session trace in the WebSocket
   attachment.
