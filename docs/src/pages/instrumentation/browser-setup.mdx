...
If you want to use the official OpenTelemetry JavaScript SDK in a browser or
...
Browsers usually send OTLP through a same-origin proxy. See
[Collectors](/instrumentation/collectors) for CORS and credential guidance.
...
class TicTacToeClient extends Client.Service<TicTacToeClient>()(
  ...
  {
    state: { awaitingPartner: S.Boolean },
    events: {},
    external: {},
  },
) {}
const State = { layer: Layer.empty }
// ---cut---

const layerTelemetry = Otlp.layerJson({
  baseUrl: "/otel",
  resource: {
...
export const runtime = Atom.runtime(
  State.layer.pipe(
    ...
    Layer.provideMerge(layerTelemetry),
  ),
)
```
