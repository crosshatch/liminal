...
- `preludeLayer` is long-lived infrastructure (databases, config, asset bindings).
...
- `Worker.make({ handler, prelude })` produces the Worker `fetch` export; combined with the Durable Object class re-export, it is everything your Worker entrypoint needs.
...
Keep the real game state in storage such as D1, Postgres via Hyperdrive, or Durable Object storage. Use attachments
