...
`upgrade(...)` forwards the current request into the Durable Object as a WebSocket upgrade. `call(...)` invokes one of the
namespace's internal methods through Durable Object RPC.
