...
Liminal is a vendor-agnostic actor framework. To host actors on Cloudflare, pair `liminal` with `effect-workerd` — a
small package of Effect services around the Workerd runtime (`Worker.make`, `Assets`, `R2`, and so on) that lets you
mount actors onto Durable Objects.

This guide walks through the simplest complete path:
...
- Wrap the actor in a `WorkerdActorNamespace` (a Durable Object class).
- Upgrade the client from an HTTP route via `Worker.make`.
- Consume the client from a browser with `Client.layerSocket(...)`.

For larger apps, split methods, events, handlers, and lifecycle into separate modules. The next guides cover that
...
The actor ties a durable name to that client protocol and defines per-client attachment state.
...
- `name` is the unique identifier for the actor instance. Use `S.String` or a branded string schema.
...
Pairing snapshot-on-connect with delta events keeps the UI story simple: connect, consume a snapshot, apply deltas —
see [Accumulator](../accumulator.mdx).
...
`R2.layerKeyValueStore` from `effect-workerd`:
...
## Wrap the actor with `WorkerdActorNamespace`

`WorkerdActorNamespace` is the Cloudflare Durable Object wrapper around your actor.
...
- `layer` is short-lived, request-local context derived from actor context.
- `handlers` is the method implementation table.
- `onConnect` runs for every newly upgraded socket.

The Durable Object binding name is supplied later, when the namespace's `.layer(...)` is applied.
...
- Each binding is supplied as a layer by calling `.layer("ENV_NAME")` with the matching Wrangler binding name. For the
  namespace, `TicTacToeNamespace.layer("TICTACTOE")` points at the Durable Object binding.
