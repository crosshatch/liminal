...
- `yield* sender.attachments` — read per-socket state
- `yield* sender.send(...)` — emit an event to one client
- `yield* sender.save(...)` — persist updated attachment state for that client
- `yield* sender.disconnect` — close that client's socket

Actors add the whole-actor operations:

- `yield* TicTacToeActor.sendAll(...)` — broadcast to every connected client
- `yield* TicTacToeActor.disconnectAll` — disconnect every connected client
...
Effect.gen(function* () {
  ...
  yield* TicTacToeActor.sendAll("GameEnded", { winner: player })
})
```
...
Effect.gen(function* () {
  ...
  yield* TicTacToeActor.disconnectAll
})
```
...
actor name and current client attachments. See
[Cloudflare Registry and Routing](./cloudflare/registry-and-routing.mdx) for the full pattern.
