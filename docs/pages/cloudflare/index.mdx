...
`liminal-cloudflare`'s `KeyValueStore.layerR2`:
...
export default Worker.make({
  handler: ApiLive.pipe(
    ...
    Effect.provide(
      Layer.mergeAll(
        KvLive,
        TicTacToeRegistry.layer("TICTACTOE_REGISTRY"),
        Assets.layer("ASSETS"),
      ),
    ),
...
Keep the real game state in storage such as R2, D1, Postgres via Hyperdrive, or Durable Object storage. Use attachments
only for per-client session state.
