...
`Hyperdrive` is the `liminal-cloudflare` wrapper around a Cloudflare Hyperdrive binding.
...
`Hyperdrive` is a single class that you import and provide a layer for using the Cloudflare env binding name:
...
## Provide the binding with `.layer(binding)`

```ts
export const PreludeLive = Layer.mergeAll(
  Db.layer.pipe(Layer.provideMerge(Layer.mergeAll(Hyperdrive.layer("HYPERDRIVE"), AppConfig.layer))),
)
```
