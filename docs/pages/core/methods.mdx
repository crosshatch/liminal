...
External methods are callable by connected clients. Internal methods are callable between Durable Object instances through
a namespace handle.
...
Shared methods such as `UpdateProfile` or `ReportUser` can live under a common `shared/methods` directory and be inserted
into a client's `external` table or a namespace's `internal` table.
