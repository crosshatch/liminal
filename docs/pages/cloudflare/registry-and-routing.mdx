...
In practice, `runLayer` is where you derive conveniences like `CurrentUserId` or `Authorization` from the actor name
and current client attachments.
...
export default Worker.make({
  handler: ApiLive.pipe(
    ...
    Effect.provide(Layer.mergeAll(ChatRegistry.layer, LobbyRegistry.layer, PreludeLive)),
...
Both sides can share the same prelude layer — either by threading `PreludeLive` through the registry's `preludeLayer`
and providing it here, or by passing it to `Worker.make(...)` as its `prelude`.
